...
- **Language**: TypeScript (Targeting ES2022+ / ESNext for modern features)
- **Standards**: "Learned Script 2025" interpreted as adherence to latest 2025 coding standards and best practices (ES2025 features where supported).
...
- **DevDependencies**: 
    - [prisma](file:///c:/Users/Usuario%20iTC/Documents/Tecnologias%20de%20la%20informaci%C3%B3n/4%20ciclo/programacion%20middleware/practica_antigravity/prisma/schema.prisma), `typescript`, `ts-node`, `@types/node`, `tap` (testing)
...
- **Structure**: Domain-Driven Design (DDD) like structure.
    - [auth/](file:///c:/Users/Usuario%20iTC/Documents/Tecnologias%20de%20la%20informaci%C3%B3n/4%20ciclo/programacion%20middleware/practica_antigravity/src/modules/auth/auth.routes.ts#7-89): [auth.routes.ts](file:///c:/Users/Usuario%20iTC/Documents/Tecnologias%20de%20la%20informaci%C3%B3n/4%20ciclo/programacion%20middleware/practica_antigravity/src/modules/auth/auth.routes.ts), [auth.schema.ts](file:///c:/Users/Usuario%20iTC/Documents/Tecnologias%20de%20la%20informaci%C3%B3n/4%20ciclo/programacion%20middleware/practica_antigravity/src/modules/auth/auth.schema.ts), `auth.service.ts`
    - [entrepreneurship/](file:///c:/Users/Usuario%20iTC/Documents/Tecnologias%20de%20la%20informaci%C3%B3n/4%20ciclo/programacion%20middleware/practica_antigravity/src/modules/entrepreneurship/entrepreneurship.routes.ts#5-51): Routes for managing businesses.
...
### Goal
Enhance the existing Swagger UI to provide complete, interactive, and secured documentation for the API.

### Steps
1.  **Configure Security Schemes**:
    *   Update [src/app.ts](file:///c:/Users/Usuario%20iTC/Documents/Tecnologias%20de%20la%20informaci%C3%B3n/4%20ciclo/programacion%20middleware/practica_antigravity/src/app.ts) to define `securitySchemes` for JWT Bearer Auth.
    *   Apply global or route-level security requirements.

2.  **Enhance Zod Schemas**:
    *   Update `*.schema.ts` files to include `.describe()` and `.openapi({ example: ... })` for better field documentation.

3.  **Route Documentation**:
    *   Add `summary`, `description`, and `security` tags to all `server.get/post` schema definitions in `*.routes.ts`.

4.  **Verification**:
    *   Start server and verify `/docs` endpoint displays the "Authorize" button and detailed schemas.
